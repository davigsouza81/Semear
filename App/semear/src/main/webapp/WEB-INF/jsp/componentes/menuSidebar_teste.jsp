<div class="col-md-2 ">
    <div class="nav-side-menu nav-justified navbar-fixed-top">
        <i class="fa fa-bars fa-2x toggle-btn" data-toggle="collapse" data-target="#menu-content"></i>
        <div class="menu-list">

            <ul id="menu-content" class="menu-content collapse out">
                <li>
                    <a href="/">
                        <i class="fa fa-user fa-lg"></i> Davi Souza
                    </a>
                </li>

                <li  data-toggle="collapse" data-target="#cursos" class="collapsed active">
                    <a href="#"><i class="fa fa-book fa-lg"></i> Cursos <span class="arrow"></span></a>
                </li>
                <ul class="sub-menu collapse" id="cursos">
                    <li class="active"><a href="/cursos/unievangelica/engenharia-de-computacao">Engenharia de Computação</a></li>
                    <li><a href="#">Engenharia de Software</a></li>
                </ul>

                <li  data-toggle="collapse" data-target="#cadastrar" class="collapsed">
                    <a href="#"><i class="fa fa-plus fa-lg"></i> Cadastar <span class="arrow"></span></a>
                </li>
                <ul class="sub-menu collapse" id="cadastrar">
                    <li><a href="#">Aluno</a></li>
                    <li><a href="#">Moderador</a></li>
                    <li><a href="#">Administrador</a></li>
                </ul>


                <li data-toggle="collapse" data-target="#service" class="collapsed">
                    <a href="#"><i class="fa fa-globe fa-lg"></i> Publicações <span class="arrow"></span></a>
                </li>  
                <ul class="sub-menu collapse" id="service">
                    <li><a href="publicacoes-pendentes">Pendentes</a></li>
                    <li>Rejeitadas</li>
                </ul>

                <li>
                    <a href="#">
                        <i class="fa fa-users fa-lg"></i> Usuários 
                    </a>
                </li>
            </ul>
        </div>
    </div>
</div>